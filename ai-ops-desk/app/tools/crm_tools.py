...
def explain_refund_state(user_id: str, user_query: str) -> str:
    ...
    # Look up refund requests from DB
    try:
        from app.rag.atlas_client import get_db
        db = get_db()
    ...
    result = {"status": 200, "explanation": explanation, "status_message": "Refund state explained successfully"}
    return json.dumps(result)
