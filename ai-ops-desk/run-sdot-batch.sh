...
# Collector (gRPC) to both Splunk O11y and Galileo.  Scenarios are mixed
...
#   ./run-sdot-batch.sh --log-stream foo  # override GALILEO_LOG_STREAM
#
# Prerequisites:
#   - OTel Collector running (./run-collector.sh)
#   - .env with OPENAI_API_KEY, MONGODB_URI
#   - ~/.cr/.cr.mongo.crm-demo, ~/.corporate-certs/env.sh
set -euo pipefail
...
# ── Parse args ──────────────────────────────────────────────────
usage() {
    echo "Usage: $0 [-n NUM_RUNS] [--no-drift] [--http-root] [--log-stream NAME] [--delay SECS]"
    exit 1
}
